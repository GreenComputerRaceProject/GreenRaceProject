...
import ohs.ServerGameScreen.runing;

public class GameScreen2 extends JPanel{

	ArrayList<HorseClass> entry = new ArrayList<HorseClass>();
	...
	class EntryData extends Thread implements Serializable{
		...
		if(set)	{
			...
			while(start) { // 말들이 달리는 반복문
				...
				for (int i = 0; i < hos.size(); i++) {
					...
					if(running.get(i) >= 1335 && running.get(i) < 1430) { // 골인 구간
						...
						hos.get(i).setText(entry.get(i).hname +"-" + a + "등");
					...
					if( running.get(i) > 1200 && running.get(i) < 1430) { // lastspeed 구간
						running.set(i, (int) (running.get(i) - entry.get(i).speed));
						running.set(i, (int) (running.get(i) + entry.get(i).lastspeed));   
					}

					if (running.get(i) > 700 && running.get(i) < 1430) { // speed 구간
						running.set(i, (int) (running.get(i) - entry.get(i).fristspeed));
						running.set(i, (int) (running.get(i) + entry.get(i).speed));
					} 
			 ...
			 */
				...
				for (int i = 0; i < hos.size(); i++) { // 8등 말이 들어왔을시 경기종료 반복문
					if(hos.get(i).getText().equals(entry.get(i).hname +"-" + 8 + "등")) {
	...
	void EntryDB() {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("select * from horse where hid = " + num);
